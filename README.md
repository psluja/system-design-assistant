...
tracked, or phoned home. _(Verified: the shipped app makes zero network calls to any non-local host —
see the [release checklist](docs/RELEASE-CHECKLIST.md).)_
...
Every capability below is a real, tested feature — see the generated
**[Feature Catalog](docs/FEATURES.md)** (20 features, each with its user story, the surfaces it ships
on, and where it's tested end-to-end) and the **[Algorithm Catalog](docs/ALGORITHMS.md)** (51
algorithmic modules with complexity, citations and invariants).
...
COIN-BC reference) — the guard against "the tool lies". Full design record — the architecture
decisions, property system, manifest contract, engine calculus and coverage map — lives in
[`backlog/docs/`](backlog/docs/) and is part of this repository.
...
- **[Feature Catalog](docs/FEATURES.md)** — every shipped feature, its user story, surfaces and tests (generated).
- **[Algorithm Catalog](docs/ALGORITHMS.md)** — every algorithmic module, its approach, complexity, citations and invariants (generated).
- **[Fidelity report](docs/FIDELITY.md)** — the honest V&V state: which numbers are validated against measured systems (~2%), which are theory-anchored or sourced by nature, and the structural limits SDA deliberately does not model (generated). The credibility backbone: [Verification & Validation strategy](docs/design/verification-and-validation.html).
- **[Design notes](docs/design/)** — deep dives (guarantee propagation, latency semantics, the solver contract, the assumption model, ideal layout, …) as self-contained HTML.
- **[Release checklist](docs/RELEASE-CHECKLIST.md)** — the pre-publication gate (no-egress, export back-compat, license audit).
- **[Backlog docs](backlog/docs/)** — the canonical architecture, property system, manifest contract and engine calculus.
...
recorded in the commit trailers.
