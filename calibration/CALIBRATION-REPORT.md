...
_Generated by `pnpm calibrate` (TASK-93). Deterministic: coordinate descent over a fixed log grid (no RNG); DES corroboration seeded (seed 7). Reproduce with `pnpm calibrate`. This report RECOMMENDS tunable values and does NOT apply them — changing a shipped content default is a separate, owner-reviewed step (see PROPOSED-DEFAULTS.json)._
...
|---|--:|--:|--:|--:|--:|
| latencySharePct | 8.50 % | 11.10 | +30.6% | 8.54 | **+0.4%** |

_Residual (RMS of fitted errors): 0.4%._

_DES tail corroboration @ 2,550 req/s (DES seed 7, 2k warmup / 8k measured, at ~0.85x the fitted ceiling): p50 39.89 ms · p95 120.14 ms · p99 166.15 ms._
