// REPORT — the generated CALIBRATION-REPORT.md and the review-only PROPOSED-DEFAULTS.json (Job 2.6 + 3).
...
export function renderReport(report: CalibrationReport): string {
  ...
  L.push('');
  L.push('_Generated by `pnpm calibrate`. ' + report.generatedNote + '_');
  L.push('');
...
export function proposedDefaults(report: CalibrationReport): string {
  return JSON.stringify(
    {
      _comment:
        'RECOMMENDATION ONLY. These are the component-default values that best fit the calibration corpus. They are NOT applied — changing a shipped content default changes every design\'s numbers, which is a separate owner-reviewed step. Regenerate with `pnpm calibrate`.',
