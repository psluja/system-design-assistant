...
const FAMILIES: readonly Capability[] = [
  ...
  },
  {
    ...
    anchors: [
      { oracle: 'differential', test: 'engine/sim/src/response.test.ts', proves: 'per-node response mean tracks its M/M/c sojourn + synchronous downstream (Burke tandem decomposition)', caveat: 'MEANS only — parallel/scatter-gather takes the max of branch MEANS, never the mean of the max; the order-statistics inflation of a synchronous fan-out join is a permanent structural limit of this analytic twin (§9), covered only by the DES' },
