...
// THE ORACLE HARNESS — random-design GENERATOR (phase 1, hardened phase 3). A seeded, deterministic instance generator that produces solver-contract-level problems —
...
// each answer; the candidate solver must match the oracle everywhere. This file makes the
...
/** The three connectivity families the generator covers (phase-1 coverage axes). A `chain` is a
...
export interface NumericInstance {
  ...
  /** The declared request classes — present ONLY on the `class` axis (a multi-commodity
...
/**
 * Plan a SCALE-axis design (item 2): a 20–30-node design that MIXES chain + fan-out + fan-in in ONE
...
export function generateNumeric(
  ...
  // The SCALE axis (item 2) is a 20–30-node chain+fan-out+fan-in MIX, a shape planDesign's single-family
  ...
  if (axis === 'budget') {
    // A cost CEILING alongside the throughput floor — the shape (a "budget" the naive corner witness
...
// ── THE GENERATOR AXIS — generate-driven load at the source ────────────────────────
...
// ── THE CLASS AXIS — multi-commodity flows over a shared sink ──────────────────────
...
const AXIS_TOPOLOGIES: ReadonlyArray<readonly [Exclude<NumericAxis, 'baseline' | 'objective-tie' | 'declined-point' | 'declined-coupled' | 'budget' | 'scale'>, readonly Topology[]]> = [
  ...
  // The CLASS axis: the HEADROOM sub-population — multi-commodity designs whose shared
...
export function generateCorpus(opts: CorpusOptions = {}): GeneratedInstance[] {
  ...
  if (opts.axes === true) {
    ...
    // The BUDGET axis: a throughput floor + a LOOSE cost ceiling — a cost-like budget the naive corner
    ...
    // The SCALE axis (item 2): a 20–30-node chain+fan-out+fan-in MIX (the CQRS-scale shape). Both solvers
    ...
    // The GENERATOR axis: generate-driven load at the source, MEAN and PEAK variants
 ...
 * at or over its capacity is the non-monotone processor-sharing boundary: native must
...
export function generateDeclinedCorpus(opts: Pick<CorpusOptions, 'perCell' | 'baseSeed'> = {}): NumericInstance[] {
  ...
  // The SATURATED class sub-population: a shared sink at/over capacity — the non-
