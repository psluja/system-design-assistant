...
// never a throw (doc-4 §1; docs/design/solver-contract.html §1). Every backward-search capability speaks
 ...
 * string-matching the human `reason` (TASK-86 named the cause in PROSE for a reader; this names it for CODE, so a
 ...
 * that MUST NOT be conflated (docs/design/solver-contract.html §3.2). Distilled from the incumbent's
 ...
 *  name it honestly (a structural decline is NOT a time limit — TASK-86 F1: never a cause dressed as a timeout). */
 ...
 * distinction between "ran out of work" and "a construct outside the provable class" is never lost (doc-4 §1:
 * uncertainty is a VALUE, and a specific one). `reason` is a plain, user-facing sentence — no stack, no jargon.
