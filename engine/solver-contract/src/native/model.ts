...
//   JS<->MiniZinc differential, doc-4 §5).
...
// THE NATIVE SOLVER — the search MODEL (TASK-79 phase 2, docs/design/solver-contract.html §3.2–§3.4). The shared
...
// pinned against (doc-4 §5). The optimum is therefore read through the SAME arithmetic the oracle certifies, not a
...
// native adapter stays free of the 17.8 MB MiniZinc bundle — that is the whole point of TASK-79.
 ...
 */
/**
 * A shared-capacity CONTENTION site under request classes (doc: request-classes §4.1, §5.2): a node whose flow key
 ...
 *  - a cyclic dependency among the freed cells is rejected (search over a cyclic flow is unsupported, doc-4 §3a).
...
export function buildModel(graph: Graph, registry: Registry, tunablesIn: readonly Tunable[], classes?: readonly RequestClass[], systemBands?: readonly SystemBand[]): Result<Model, readonly string[]> {
  ...
  // The shared-capacity CONTENTION sites (doc: request-classes §4.1, §5.2). Only present under declared classes:
