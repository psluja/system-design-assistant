...
// THE NATIVE SOLVER — the search ENGINE. This is
...
// the same least-fixpoint the hot path runs and the MiniZinc projector is differentially pinned against), so the search only has to be exact about WHICH knob values it picks; correctness of the arithmetic is the
...
// re-evaluated verdicts agree with the incumbent's. Real comparisons use the CPU-proves discipline: the engine computes the value, the search only compares it to a bound.
//
...
// This is the SDA-content case that made the CQRS dogfood decline. Where a design falls OUTSIDE the
 ...
 * the non-monotone processor-sharing boundary the native solver must decline. Total
 ...
 * the native solver must DECLINE rather than guess. CONSERVATIVE by design: over-
 ...
 *  reports. */
/** Honest true-cause reasons a monotone search returns with a `did-not-converge`: the search NAMES
...
function prepare(registry: Registry, graph: Graph, tunables: readonly Tunable[], headroom: Headroom | undefined, signal: AbortSignal | undefined, classes: readonly RequestClass[] | undefined, systemBands?: readonly SystemBand[]): Prepared {
  ...
  // OUT OF THE MONOTONE CLASS when a shared node can SATURATE: the processor-sharing
