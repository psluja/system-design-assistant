// The INCUMBENT adapter — the first implementation of the solver contract (// §3, migration step 3). It wraps today's code: the cell-network `createEngine` (evaluate/optimize/repair/
...
export function makeIncumbentAdapter(deps: IncumbentDeps): SolverBindings {
  ...
  // The synchronous hot path — always provided; it needs no injected solver. CLASS-AWARE: it calls the network `evaluate` directly so a request carrying classes folds the flow per class; absent
...
/** Whether a search request declares request classes — routes to the per-class emitter
