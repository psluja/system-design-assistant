...
/** A derived key, computed by an authored relation that reads other keys. */
export interface Relation {
  readonly produces: Key;
  readonly reads: readonly Key[];
  /** Authored expression source (the readable MiniZinc-subset). Opaque to engine/core. */
  readonly expr: string;
}

/** One (owner, key) slot: an input assumption or a derived relation. */
export type Cell =
...
/**
 * One k6/Gatling-style STAGE — one step of a generator's schedule. Over `durationS`
...
/**
 * One periodic shaping CYCLE — an ordered `stages` schedule that REPEATS every
 ...
 * FLOW-flagged quantity as it crosses a port. Purely MATH data: `value` is the
 ...
 * - `generate`: the port ORIGINATES flow instead of reshaping it — the sixth member.
...
export function applyTransform(t: Transform | undefined, x: number): number {
  ...
  switch (t.kind) {
    ...
    case 'generate': // identity at the edge seam: a generator ORIGINATES flow at its NODE (the level enters the
      ...
      // crossing the port is already the node's emission. The port function reshapes nothing. Presenters labelling a generate port's wire therefore show the
...
export interface Edge {
  ...
  /** OPTIONAL per-WIRE OUT-side transfer function — a ROUTING SPLIT that a
  ...
  readonly transform?: Transform;
  /** OPTIONAL categorical GUARANTEE contribution this EDGE declares. Content
...
export type GraphError =
  ...
  /** A `generate` sits on an IN port: a generator ORIGINATES flow, so it lives on
...
export function buildGraph(
  ...
  for (const p of parts.ports) {
    ...
    // A generator originates flow, so it lives on an out/bi port only — an in-port generate
 ...
 * discipline: every rule names the fix, never a silent
