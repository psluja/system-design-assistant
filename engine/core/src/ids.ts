 ...
 * a `PortId` will not type-check where a `NodeId` is expected (doc-4 §2: illegal states unrepresentable).
 */
export type NodeId = string & { readonly __brand: 'NodeId' };
export type PortId = string & { readonly __brand: 'PortId' };
export type EdgeId = string & { readonly __brand: 'EdgeId' };

/** A registry key id. The engine knows keys exist; their meaning is content (doc-4 §2). */
export type Key = string & { readonly __brand: 'Key' };

/** A request-class id (doc: request-classes §3). A CLASS is a named flow (a commodity): its own origins
