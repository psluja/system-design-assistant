...
// guarantees — consistency, ordering, delivery — COMPUTABLE with certainty (doc: guarantee-propagation §2).
//
...
// opaque `DimensionToken` strings that content declares in the registry's categorical section (doc-4 §2).
//
...
// ONE mechanism for all three dimensions; the meet IS the monotone OR (doc: guarantee-propagation §2 delivery).
 ...
 * when its guarantee is honestly not known (doc: guarantee-propagation §3 "refused → unknown"). A meet that
