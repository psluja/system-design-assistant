 ...
 * concern (doc-2 §2, default js-quantities); engine/core only carries the tag.
 */
...
/** How a key's band is shaped (doc-2 §4). Declared per key in the registry. */
export type BandShape = 'point' | 'minTargetMax' | 'percentiles';

/** An SLO band — the acceptable envelope for a key (doc-2 §4). */
export type Band =
...
/** The value of an input cell: a fixed quantity (config/limit) or an SLO band (doc-2 §3). */
