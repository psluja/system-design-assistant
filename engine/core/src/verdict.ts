 ...
 * from honest ignorance (`unknown` / `did-not-converge`). Never a bare boolean (doc-4 §6).
 */
export type Status = 'ok' | 'warning' | 'violation' | 'unknown' | 'did-not-converge';

/** One link in the chain of upstream cells/constraints that drove a verdict (doc-4 §6). */
...
/** A ranked, actionable fix derived from a tunable (doc-2 §4). Lower `rank` = offered first. */
export interface Remediation {
  readonly action: string;
  readonly rank: number;
}

/** The engine's honest, explanatory output for one checked property (doc-4 §6). */
export interface Verdict {
  ...
  /** With request classes declared (doc: request-classes §4.2), the class this verdict is FOR — a per-class
