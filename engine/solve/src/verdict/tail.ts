 ...
 * caller builds one from a DES run, keeping engine/solve decoupled from engine/sim.
 */
 ...
 * `unknown` the scalar forward pass must return for them. A target is breached when
  ...
  for (const node of graph.nodes.values()) {
    for (const cell of node.cells) {
      ...
      for (const [name, target] of cell.value.band.targets) {
        ...
        // ε-tolerant: a tail within float noise of its target is AT it, not a
