 ...
 * computed value of that key against the band → ok / warning / violation / unknown (doc-4 §6).
 * Percentile bands are 'unknown' until the DES computes true tails (doc-4 §3b). `cause` and
...
export function evaluateBands(
  ...
  for (const node of graph.nodes.values()) {
    for (const cell of node.cells) {
      ...
      // PER-CLASS PERSPECTIVES (doc: request-classes §4.2): under declared classes, a non-flow, non-local key
...
function statusForBand(v: number, band: Band): Status {
  switch (band.shape) {
    ...
    case 'percentiles':
      return 'unknown'; // true tails need the DES, not a scalar (doc-4 §3b)
  }
}
