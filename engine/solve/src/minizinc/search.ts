 ...
 *  off `out(node, key, class)` (doc: request-classes §5.1). Absent ⇒ the class-blind cell (a flow total / today).
 *
...
/**
 * Rewrite every processor-sharing split in a cell system to its headroom-region linear form (doc: request-classes
 * §5.3). The split `served(N,K,C) = offered · cap / max(total, cap)` (build.ts psSplit) divides by a VARIABLE, so
 ...
 * free cells would need the post-fixpoint+minimize encoding (doc-4 §3a), which conflicts with a cost
...
function compile(graph: Graph, registry: Registry, tunablesIn: readonly Tunable[], classes?: readonly RequestClass[]): Result<Compiled, readonly string[]> {
  ...
  // headroom-region form `min(cap, offered)` (doc: request-classes §5.3): EXACT wherever the search operates — a
  ...
  if (hasCycle(varDeps)) {
    return { ok: false, error: ['search over a cyclic flow is not supported in this slice (doc-4 §3a)'] };
  }
 ...
 * HARD constraints, over the freed `tunables` (doc-4 §4 `optimize`). Solving it yields the tunable
...
function hardBand(t: string, band: Band): string[] {
  switch (band.shape) {
    ...
    case 'percentiles':
      return []; // a tail SLO needs the DES, not a scalar (doc-4 §3b)
  }
}
...
/**
 * The UNSAT explainer (doc-4 §4): re-encode every band as SOFT — a non-negative penalty makes up any
...
/**
 * The `repair` mode (doc-4 §4): given a graph that violates its SLOs, find the MINIMAL change to the
