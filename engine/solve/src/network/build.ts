...
/**
 * A REQUEST CLASS as the network builder consumes it (doc: request-classes §3): a named flow (a commodity)
 ...
 * accessor for the per-node output of a key. This is engine-internal (doc-4 §10); component authors
...
// Class-only flow cells (doc: request-classes §4.1): a class's origin injection, its offered load (= inflow + origin),
...
// A generator port's SOURCE TERM for a flow key (doc: load-curves §3): the level a `generate` port originates,
...
/**
 * Apply a port's transfer function to a value EXPRESSION, as pure arithmetic (doc: flow-transformations §3).
...
function transformExpr(t: Transform | undefined, x: Expr<CellId>): Expr<CellId> {
  ...
  switch (t.kind) {
    ...
    case 'generate': // identity at the edge seam (doc: load-curves §3): a generator ORIGINATES flow at its NODE —
 ...
 * (doc: request-classes §4.1): `served(N,K,C) = offered(N,K,C) · capacity / max(totalOffered, capacity)`.
...
/**
 * Find a cycle in ONE class's edge subgraph, or null if it is acyclic (doc: request-classes §4.2). A class's own
...
export function buildNetwork(graph: Graph, registry: Registry, classes?: readonly RequestClass[]): Result<Network, readonly string[]> {
  ...
  for (const edge of graph.edges.values()) {
    ...
    // RESOLUTION ORDER for the OUT-side (doc: flow-transformations-r2 §5): the WIRE's own transform WINS over the
  ...
  // Request classes (doc: request-classes §3): validate membership + origins, and enforce per-class acyclicity.
  ...
  if (hasClasses) {
    ...
    for (const rc of declaredClasses) {
      ...
      if (cyc !== null) errors.push(`class "${String(rc.id)}": cyclic membership ${cyc.map((n) => String(n)).join(' → ')} — a request class must be acyclic (doc: request-classes §4.2)`);
    }
  }
  ...
  for (const node of graph.nodes.values()) {
    const n = node.id;
    // THE GENERATOR PORTS (doc: load-curves §3): out/bi ports of THIS node carrying `generate` with a level > 0
    ...
    for (const k of keysInPlay) {
      ...
      if (localOnly) {
        ...
        // THE GENERATOR SOURCE TERM (doc: load-curves §3) — flow keys only, and only where a generator port sits:
        ...
        // ── PER-CLASS FLOW CELLS (doc: request-classes §4.1–4.2) ──
 ...
 * depth limit on long chains (doc-4 §10: canonical ordering for the fixpoint).
 */
