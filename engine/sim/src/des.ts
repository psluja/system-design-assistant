...
export interface SimOptions {
  ...
  /**
   * FLOW-SCOPED LAG pairs to measure. Each is a (source, terminal) station pair
...
/**
 * One node's RESPONSE from its OWN perspective: the time from a job's ARRIVAL at
...
/**
 * One DECLARED flow's LAG distribution: the wall-clock from a lineage's arrival at
...
export interface SimResult {
  ...
  /** Sojourn percentile, p ∈ [0,1] (nearest-rank) — the true tail. */
  sojournPercentile(p: number): number;
  readonly stations: readonly StationStats[];
  // Retry-feedback outcome accounting. Measured over the same window as everything
  ...
  /** Every station's response perspective, from this one run. Empty ⇒ no run yet. */
  ...
  /** Every DECLARED lag pair's distribution, from this one run. Empty when
 ...
 *
 * Retry state rides on the REQUEST, the caller's unit of work: `policy` is the caller's
...
interface Job {
   ...
   * station's synchronous subtree completes. Overwritten each time the fork
   *  enters a new station; undefined only before its first entry. */
  frame?: Suffix;
  /** FLOW-SCOPED LAG stamps: source station id → the time THIS lineage arrived at
...
/**
 * One node-visit's RESPONSE suffix: the clock from a job's ARRIVAL at a station to
...
type SimEvent =
  ...
  // Station wait-deadline abandonment: a job has waited at THIS station past its
...
interface StationState {
  ...
  area: number;
  areaBusy: number;
  // Per-node RESPONSE sampling. `respReservoir` is a BOUNDED uniform sample of this
 ...
 * (time, then sequence), so a seed fixes the output byte-for-byte. It answers the
...
export function simulate(network: QueueingNetwork, opts: SimOptions): SimResult {
  ...
  // A THIRD disjoint stream for FLOW-SCOPED LAG reservoir sampling, seeded off the
  ...
  // FLOW-SCOPED LAG bookkeeping. Precompute, from the DECLARED pairs only, the set
  ...
  const lagKey = (source: string, terminal: string): string => `${source}\x00${terminal}`;
  ...
  const enter = (s: StationState, job: Job, syncParent: Suffix | undefined): void => {
    ...
    s.n += 1;
    // Open this visit's RESPONSE suffix: the clock starts now, at arrival here.
    ...
    // FLOW-SCOPED LAG: this job ARRIVED here (admitted — a dropped fork returned
  ...
  while (exits < maxExits) {
    ...
    // Close this node's RESPONSE suffix. Its own service is now done, so it stops
  ...
  // FLOW-SCOPED LAG results — one entry per DECLARED pair, its reservoir sorted once
