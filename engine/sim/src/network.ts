 ...
 *
 * `maxQueueWaitMs` is a STATION-LEVEL wait deadline: a job that
...
/**
 * A caller's per-attempt retry policy. Pure timing math — no domain vocabulary. A job
 ...
 * retry policy that travels with every job this source injects. It rides on the
...
export interface ArrivalSource {
  ...
  /**
   * OPTIONAL time-of-period MODULATION of this source's arrival rate — a periodic,
 ...
 * the per-edge traffic transfer of the flow model. It is realised HONESTLY as a
 ...
 * async target begins a fresh response perspective of its own. */
