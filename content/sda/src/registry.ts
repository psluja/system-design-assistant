...
// The seed PROPERTY REGISTRY (doc-5): the canonical keys a credible design tool reasons about, each
...
export const keys = {
  ...
  // how a node combines the RESPONSE latencies of its SYNCHRONOUS downstream calls (doc-15): 0 = sequential
  ...
  // STATION WAIT DEADLINE (doc: retry-feedback §3 "reneging") — the longest a request may WAIT AT this node for a
  ...
  // data-transfer (egress) cost — the most-missed line on a real bill (doc-10). A node sending `payloadBytes`
  ...
  // wrapper (behaviors.ts `withOrigin`), so downstream sees it as ordinary inflow. See doc-2 "universal
  // traffic origins" and the flow-model comment on `throughput` above.
  assumedRps: Key('assumedRps'),
  // RETRY POLICY (doc: retry-feedback) — the caller's per-attempt retry behaviour, as three node-local config
  // knobs on the CALLING node (client / origin service). They shape ONLY the DES: a retry loop is non-monotone
  // feedback (load ↑ → timeouts ↑ → load ↑), a "question about time" (doc-8 §0), so the scalar pass never reads
  ...
  // DES-FED OUTCOME keys (doc: retry-feedback §3) — SLO-only, answered ONLY by the simulator (like tailLatency).
...
export const registry: Registry = registryOf([
  ...
  // `flow: true` marks throughput as THE quantity port transforms act on (doc: flow-transformations): a port's
  ...
  // input read by the responseLatency projector (doc-15). Inert in the cell-network — it selects a fold, not a value.
  ...
  // RETRY-POLICY inputs (doc: retry-feedback). Node-LOCAL point knobs on the CALLING node; they never flow and
...
// ─── THE ROLE AXIS (doc: assumption-model §2) ─────────────────────────────────────────────────────────────────
...
/**
 * The role of every registry key (doc: assumption-model §2), keyed by key id. EXHAUSTIVE — roles.test.ts asserts
