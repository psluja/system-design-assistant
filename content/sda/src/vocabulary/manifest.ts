...
// @algorithms engine/solve/src/network/build.ts (scalar folding), content/sda/src/analysis/sim.ts (DES route
...
// A component MANIFEST is pure DATA: config knobs/limits, derived relations, SLO bands and
...
export interface ManifestPort {
  ...
  /** OPTIONAL per-port traffic transfer function: the manifest DEFAULT emission
  ...
  /** OPTIONAL categorical GUARANTEE contribution this port declares — the token,
...
export interface ManifestConfig {
  ...
  /** PROVENANCE as DATA, not a code comment. A config's origin was, until now, only in a
...
/** An OPTIONAL uncertainty RANGE on an instance config value — the honest
...
export interface Instance {
  ...
  readonly bands?: readonly ManifestBand[];
  /** PER-INSTANCE port transform overrides, by PORT NAME. The owner requirement:
  ...
  /** PER-INSTANCE uncertainty RANGES, keyed by CONFIG KEY — the additive twin of
...
export interface Wire {
  ...
  /** OPTIONAL per-WIRE OUT-side transform override — a ROUTING SPLIT a per-port
  ...
  readonly transform?: Transform;
  /** OPTIONAL categorical GUARANTEE the WIRE itself declares. Content decides what
...
export type InstantiateError =
   ...
   * build problem instead of silently sampling a poisoned range. */
...
/** One resolved GENERATOR on a placed instance: the port it sits on, its `level` (the
 ...
 * reconciled class-blind origin figure. */
...
export function instantiate(
  ...
  // RANGE SANITY: an uncertainty range must bracket its point (lo ≤ hi, a
  ...
  // Under DECLARED request classes the per-class origins are authoritative — the
  ...
  for (const inst of instances) {
    ...
    for (const c of m.config ?? []) {
      ...
      // GENERATORS reconcile the same way: with no explicit instance config, the cell is
    ...
    // the origin. Built-ins get the cell from `withOrigin`'s default via the loop above.
  ...
  // A wire's OPTIONAL per-wire transform rides onto its engine edge as the
