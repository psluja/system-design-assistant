...
// @algorithms content/sda/src/load-stages.ts (the multi-cycle rate product), content/sda/src/time-sweep.ts (the
//   Tier-1 quasi-static sweep + per-node peak), content/sda/src/two-tier.ts (propose/prove), content/sda/src/sim.ts (DES lowering)
...
// THE LOAD-STAGES HOME (doc: load-stages §16.3) — the SINGLE SOURCE OF TRUTH for the two-tier evaluator's
...
//
// THE MODEL (doc: load-stages §4-§5): a generator carries `level` (the BASELINE rate, the `×1` the multipliers
...
/**
 * The absorbed stress-spike defaults (doc: load-stages §16.3 — "absorbs STRESS_DEFAULTS"). The one-shot spike
...
// @where-tested content/sda/src/load-stages.test.ts
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────
...
/**
 * Lower ONE cycle to the time engine's {@link RateProfile} (doc: load-stages §9) — the baseline-anchored
...
/**
 * The generator's instantaneous rate λ(t) = level · Π_cycles m_c(t) (doc: load-stages §5, product WITHIN a
 * generator). Empty/absent cycles ⇒ the flat baseline `level` (the sacred identity). The scalar product of a
...
/**
 * The shortest STAGE the fine sweep must resolve (doc: load-stages §16.3 A) — the smallest ramp/hold `durationS`
...
/**
 * The auto-derived observation span (doc: load-stages §10.4, §16.3 B) — the slowest cycle's period × `spanRepeats`
 ...
 * `count` evenly-spaced points (doc: load-stages §11, the editor preview). The ONE sampler the AUTHORING previews
...
/**
 * The DERIVED MEAN rate over the observation span (doc: load-stages §7) — `level × mean(Π cycles)`, the honest
...
/**
 * The DERIVED PEAK rate over the observation span (doc: load-stages §7) — `level × peak(Π cycles)`, the worst
 ...
 * piecewise-linear profile over one slowest period (doc: load-stages §5 honest limit, §9).
 */
...
/** The names of the shipped presets (doc: load-stages §16.2 E). */
...
/**
 * A ONE-SHOT shape's natural period (doc: load-stages §16.3 A/B) — its own total scheduled duration (Σ durationS),
...
/**
 * The six presets (doc: load-stages §16.3). `flat` has NO cycles (the ×1 identity — the sacred pin). The `spike`
