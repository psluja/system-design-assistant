...
// THE TIER-1 SWEEP (doc: load-stages §10.2) — "the sweep proposes, the DES proves", applied to time. This is the
...
/** The Tier-1 sweep result (doc: load-stages §10.2). Basis is `analytic (quasi-static)` — never measured dynamics. */
...
/**
 * Run the Tier-1 analytic time-sweep on a design (doc: load-stages §10.2). Returns `undefined` when the design
 ...
 *  not only the steady baseline (doc: load-stages R4 — "where do I see which nodes fail SLO after the peaks"). */
 ...
 * saturates it reads its worst-window ρ, not the mean (doc: load-stages R4). A node whose ρ never varies simply
