...
import type { Instance, Wire } from '../vocabulary/manifest';
...
// REQUEST CLASSES — the authored, document-shape declaration. A CLASS is a
...
/** A per-class origin: the rate THIS class injects at a node — its own `assumedRps`. */
...
/**
 * A request-class DECLARATION as it rides on the project document. Pure DATA: an `id`
 ...
 * classes carve the shared (possibly cyclic) topology into per-class ACYCLIC slices, so
...
export function cyclicFlowDiagnosis(wires: readonly Wire[]): { readonly cycle: readonly string[]; readonly message: string } | undefined {
  ...
  return {
    ...
    message:
      ...
      `Break it with REQUEST CLASSES: declare each commodity as its own class (declare_scenario is unrelated — use the request-class tools) so the shared topology can loop while every class's path stays acyclic — e.g. the follow-up writes flowing back are a distinct class from the originating request.`,
  };
}
