...
// @docs none
// @e2e content/sda/src/analysis/tail.e2e.test.ts, content/sda/src/analysis/response-latency.e2e.test.ts,
...
//
// LATENCY SEMANTICS v2. A node's latency SLO is judged against its RESPONSE
// latency — its own queued sojourn PLUS the responses of everything it SYNCHRONOUSLY calls (an async hop cuts the
// wait) — the exact number the node card shows (`responseLatency`). It is NO LONGER the source→node
...
export function realAwareVerdicts(
  ...
  // accumulation. `responseLatency` is the exact quantity the card renders, so what is judged equals
...
/** The whole-system outcome the DES measured. These are SYSTEM metrics — the useful work
