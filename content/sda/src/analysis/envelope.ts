...
//   (differential-tested).
// @where-tested content/sda/src/analysis/envelope.test.ts, content/sda/src/analysis/envelope-des.e2e.test.ts
...
// @algorithms content/sda/src/analysis/envelope.ts, engine/solver-contract/src/native/search.ts,
...
import { instantiate, type Instance, type Manifest, type Wire } from '../vocabulary/manifest';
import { nodeQueues } from './queueing';
import { TARGET_UTILIZATION } from '../catalog/behaviors';
import { NO_ORIGIN_REASON } from './system';
import { originNodes, scaledInstances, type OriginNode } from './sweep';

// THE CAPACITY ENVELOPE — the DEFAULT answer, needing NO declared demand: the maximum
...
/**
 * Compute the capacity envelope of a design. PURE aside from the injected `optimize`
