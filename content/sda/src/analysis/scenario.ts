...
import { isFactAssumption, keys, roleOf } from '../vocabulary/registry';
...
// @algorithms engine/solve/src/fixpoint/solve.ts, content/sda/src/analysis/envelope.ts (the trio derives
//   demand from the envelope)
// @docs none
// @e2e app/web/src/worlds.e2e.test.ts, app/mcp/src/cqrs-scale.e2e.test.ts
// @status shipped

// NAMED WORLDS (scenarios) — the assumption model's view two. A scenario names a concrete
...
export function overrideRoleProblem(node: string, key: string, instances: readonly Instance[], wires: readonly Wire[]): string | null {
  ...
  return `"${key}" is ${where}; a scenario may override only fact-assumption inputs`;
}
