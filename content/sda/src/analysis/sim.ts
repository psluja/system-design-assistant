...
// @where-tested content/sda/src/analysis/sim.e2e.test.ts, content/sda/src/analysis/queueing.e2e.test.ts,
//   content/sda/src/vocabulary/transform.e2e.test.ts, content/sda/src/vocabulary/generator.e2e.test.ts
...
/**
 * The caller-side retry policy declared on a node, or undefined when it sets no deadline.
 ...
 * per-hop / per-wire retry policies are a declared non-goal, revisited with a named case.
 */
...
/**
 * A generator's cycles lowered to the time engine's RateProfile — the baseline-anchored
...
/**
 * The DES arrival for a node's generators — the total baseline-anchored stream
...
/**
 * Project a design graph onto a queueing network for the time engine. This is the
...
export function toQueueingNetwork(graph: Graph): QueueingNetwork {
  ...
  for (const node of graph.nodes.values()) {
    ...
    if (emit > 0) {
      // The retry policy is a fact of THIS caller's code, so it rides on the traffic this
      // node ORIGINATES. Absent (timeoutMs=0) ⇒ no field ⇒ today's DES.
      const attemptPolicy = attemptPolicyOf(node);
      // THE CYCLES: the node's generators carry the SHAPE; `level` is the BASELINE, so the
    ...
    // CONNECTION POOL → FINITE-server M/M/c: a fixed-throughput resource-pool
