...
// synthesize() — the "run backwards from scratch" capstone (doc-4 §4): generate → size → rank, the
...
// (docs/design/solver-contract.html §3), injected by the composition root — the SAME `SolverBindings` the
// backward-search tools use, so synthesis can never drift onto a second solver seam (TASK-79 phase 2.5).
