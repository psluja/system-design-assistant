...
import { clientOut, triggerIn } from './port-roles';
import { writerGuarantees } from './guarantees';
...
// A SEED CATALOG of archetype-first manifests (doc-6). Numbers are illustrative starting points, not
...
export const manifests: Readonly<Record<string, Manifest>> = withOverflow(withOrigin({
  ...
  // flags the rejected excess = max(0, offered − 10,000); those rejects ARE the 429s (see doc-8 documented-limits).
  ...
  // load-independent verdict; burst absorption is a DES/time question (doc-8 NOT-covered).
  'gateway.api': {
    type: 'gateway.api',
    ports: [
  ...
  // A cheaper datastore alternative — genuinely "cheaper but more LIMITED", never a free win (TASK-42): it
