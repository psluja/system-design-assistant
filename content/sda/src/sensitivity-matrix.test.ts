...
// about TIME, doc-8 §0) go through a seeded `simulate` with CI-tolerant thresholds.
//
...
const MATRIX: readonly Row[] = [
  ...
  // ── DES-only knobs (a retry loop is non-monotone feedback — a question about TIME, doc-8 §0; answered ONLY by
  ...
  //    PAY-PER-USE cost (doc: flow-transformations). The transform reshapes the load that reaches the sink, so we
  ...
  // ── DES: STATION WAIT DEADLINE + QUEUE DRAIN → dropped / errorRate ────────────────────────────────────────
  {
    ...
    // the deadline reveals the honest capacity ceiling by shedding the excess as timeouts. (doc: retry-feedback §3.)
...
// ── FLOW-SCOPED LAG (doc: latency-semantics-v2 §3) — a DES-ONLY, PAIR-scoped observable the per-node metric grid
