...
export const commonManifests: Readonly<Record<string, Manifest>> = withOverflow(withOrigin({
  ...
  // can grow when session state PINS a connection — pinning is workload-dependent and NOT modelled, see).
  ...
  // an invented number, so it stays out of the math.
  'proxy.rds': {
    type: 'proxy.rds',
    ports: [
  ...
  'cache.redis': {
    type: 'cache.redis',
    ports: [
      // A cache read with no declared invalidation is EVENTUAL — sourced behaviour
  ...
  'db.postgres': {
    ...
    ports: [
      ...
      // behaviour carried as data — a request path terminating here provides `strong`.
  ...
  // A READ REPLICA of a relational primary — the exemplar of an EVENTUAL read.
  ...
  'queue.sqs': {
    ...
    ports: [
      ...
      // consumers poll the queue. HINT: ReceiveMessage returns AT MOST 10 messages
