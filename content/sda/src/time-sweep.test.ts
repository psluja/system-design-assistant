...
// TIER-1 ANALYTIC TIME-SWEEP (doc: load-stages §10.2) — the quasi-static sweep proposes the worst window and the
...
// WINDOW RESOLUTION resolves the shortest FEATURE, not only the fastest PERIOD (doc: load-stages §16.3 A). The bug:
...
// PEAK-AWARE PER-NODE LOAD (doc: load-stages R4). peakLoadByNode projects, per node, its worst ρ over the season and
