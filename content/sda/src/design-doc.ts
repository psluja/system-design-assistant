...
// @algorithms content/sda/src/system.ts, content/sda/src/queueing.ts, content/sda/src/sweep.ts,
//   engine/sim/src/des.ts (measured verdicts via doc-sim)
// @docs docs/design/design-doc-v2.html, docs/design/final-acceptance-design-doc.html,
//   docs/design/sample-generated-design-doc.html
...
import { localContribution, localOwnAvailability, requestFlows, systemSummary, type ValueFn } from './system';
import { systemPromiseVerdicts, type SystemPromise } from './system-promise';
import { availabilityTier, RELIABILITY_SOURCES, reliabilityAdvice } from './reliability';
import type { Instance, Manifest, ManifestBand, Wire } from './manifest';
import {
  buildDocModel,
  pathAvailabilityFor,
  type AssumptionRow,
...
// design document (doc-7 sections) with the computed NFR numbers filled in, a C4 container view, a
...
// sections the tool does not model are flagged "author required" (doc-7's gating purpose: flag the absence
// of the resilience / security / cost / alternatives hallmarks). It lives in content, not the engine,
...
export interface DesignDocInput {
  ...
  /** Per-node REAL request→response latency in ms — `responseLatency(...)` (doc-15): a tier's own wait plus the
  ...
  /** The RETRY OUTCOME the DES measured (doc: retry-feedback §3), present ONLY when a retry policy is declared:
  ...
  // ── DocModel v2 extensions (doc: design-doc-v2). All OPTIONAL, so every existing caller keeps compiling; a
  ...
  /** Per-flow guarantee verdicts (doc: guarantee-propagation §4) — DATA in (the caller runs `guaranteeVerdicts`
  ...
  /** Per-flow LAG verdicts (doc: latency-semantics-v2 §3) — DATA in (the caller runs `lagVerdicts`). Absent/empty
  ...
  /** Per-node SIMULATED response percentiles (ms) from a DES run (doc: latency-semantics-v2 §4) — DATA in (the caller
   ...
   *  label (the TASK-87 scope-labelling extended). Absent/empty ⇒ both sections render exactly as before. */
...
const transformVerb = (t: Transform): string => {
  switch (t.kind) {
    ...
    case 'generate':
      // A generator ORIGINATES flow (doc: load-curves §3) — the narrative names the level (the day's mean rate).
...
export function generateDesignDoc(input: DesignDocInput): string {
  ...
  if (slos.length > 0 || sysProms.length > 0) {
    ...
    h('|---|---|---|---|---|');
    // SYSTEM-scoped rows FIRST (the global promises lead — TASK-87's scope labelling extended with `system`):
  ...
  h('');
  // PROPAGATION LAG (doc: latency-semantics-v2 §3) — the async-INCLUSIVE end-to-end view BESIDE the flow table: each
  ...
  // RETRY POLICY & GOODPUT (doc: retry-feedback). A retry policy is declared as caller-side config (timeoutMs > 0)
  ...
  // ACTIVE flow transforms (doc: flow-transformations) — where a port does NOT relay 1:1, the deliverable must say
  ...
  // Per-node SIMULATED response PERCENTILES (doc: latency-semantics-v2 §4) — the DES tail (p50/p95/p99) a caller of
  ...
  // §9–12 the doc-7 hallmark sections the tool does not model — flagged, never silently absent.
  ...
  // Completeness checklist — doc-7's gating function made explicit.
  h('## Completeness (doc-7 gating sections)');
  h('');
  h('| Section | Source |');
  h('|---|---|');
  ...
  // ── DocModel v2 sections (doc: design-doc-v2). APPENDED after the legacy doc-7 body so every pre-existing
...
/**
 * Render the design document as the self-contained HTML DELIVERABLE (doc: design-doc-v2 R2) — the human-facing
...
/** Render the §7b guarantees section as a Markdown table (doc: guarantee-propagation §4): per-flow requirement,
...
/** The doc-7 sections SDA does not model — emitted as flagged stubs so the gating hallmarks are visible. */
