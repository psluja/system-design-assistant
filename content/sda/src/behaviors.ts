...
// DATA-TRANSFER (egress) cost — the most-missed line on a real bill (doc-10: 20-40% of spend). A node at the
 ...
 * TRANSIENT, not a steady-state ceiling, so it is NOT modelled here — recorded in doc-8 as honestly uncovered
...
// cross-region replication data transfer — an ESTIMATE (exact figures are workload/region-dependent, doc-8 §4).
//
...
//     separate, genuinely-extra region and is a documented follow-up, not this fold — see doc-8.)
 ...
 *  (exact spend is region/workload-dependent, doc-8 §4): a further +0.3× on top of the ×2 Multi-AZ ⇒ a TOTAL factor
...
/** The CALLER-SIDE RETRY POLICY knobs, as editable config DATA (doc: retry-feedback §2). A retry policy is a
 ...
 *  time, doc-8 §0). Applied explicitly to the caller manifests rather than every node, so the vocabulary stays
