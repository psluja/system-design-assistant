...
// carry the COMPUTED numbers (never hand-entered) and the design-document sections, so a single source serves both
...
describe('design-doc generator (from the verified model)', () => {
  ...
  it('emits the design-document sections', () => {
  ...
  it('flags the hallmark sections it does not model as author-required', () => {
  ...
  // RETRY POLICY & GOODPUT in the deliverable. When a caller declares a retry
