...
// THE LOAD SWEEP (doc: design-doc-v2 §5) — the "how does end-to-end latency respond as traffic rises?" chart series
 ...
 *  so the capacity ENVELOPE (doc: assumption-model §3) reuses the SAME origin detection the sweep uses — one
...
export function originNodes(instances: readonly Instance[], catalog: Readonly<Record<string, Manifest>>): OriginNode[] {
  ...
  for (const inst of instances) {
    // A GENERATOR port (doc: load-curves §3) is the primitive origin declaration; `assumedRps` is its sugar. The
 ...
 *  Exported for the ENVELOPE (doc: assumption-model §3): scaling a SUBSET of origins (pass just one) frees a single
