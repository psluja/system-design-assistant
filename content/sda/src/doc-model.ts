...
// THE DOC MODEL (doc: design-doc-v2 §4) — the verified model turned into EXHAUSTIVE TYPED DATA: the sections of
...
export interface DocModelInput {
  ...
  /** Per-flow LAG verdicts (doc: latency-semantics-v2 §3) — DATA in (the caller runs `lagVerdicts`), never computed
   *  here. Absent/empty ⇒ the propagation-lag block is omitted (no-filler). */
  readonly lagVerdicts?: readonly LagReqRow[] | undefined;
  /** Per-node RESPONSE PERCENTILES (ms) from a DES run (doc: latency-semantics-v2 §4) — DATA in (the caller reads
...
export interface CapacitySection {
  ...
  /** Per-tier response latency (doc-15), when supplied. */
  readonly responseLatencyByNode?: Readonly<Record<string, number>>;
  /** Declared flow-scoped LAG deadlines + verdicts (doc: latency-semantics-v2 §3), when the caller supplies them.
  ...
  /** Per-node SIMULATED response percentiles (doc: latency-semantics-v2 §4) for the REQUIREMENT-BEARING nodes — the
...
/** One per-flow qualitative-guarantee row (doc: guarantee-propagation §4 "per-flow table"): the requirement, the
...
/** One per-flow LAG row (doc: latency-semantics-v2 §3) — a declared propagation deadline (source→terminal, async
...
export function buildDocModel(input: DocModelInput): DocModel {
  ...
  // branch's accumulated cell — and the renderers label the scope `system` (TASK-87's labelling extended).
...
function buildAssumptions(input: DocModelInput, instById: Map<string, Instance>): AssumptionRow[] {
  ...
  for (const w of wires) {
    ...
    // A GENERATOR is an ORIGIN, not a reshaping (doc: load-curves §3): its level already appears in the register
  ...
  // Declared GUARANTEE contributions (doc: guarantee-propagation §4 "every declared contribution appears in the
  // assumptions register"). Each per-port token is an assumption the guarantee verdicts rest on, with its OWN
...
function buildCapacity(
  ...
  // Per-node SIMULATED response percentiles (doc: latency-semantics-v2 §4) — the DES tail, shown for the
