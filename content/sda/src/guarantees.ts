...
// The seed CATEGORICAL vocabulary (doc: guarantee-propagation §2) — the three qualitative-guarantee dimensions
...
/** One dimension's sourced claim for a port/edge (doc: guarantee-propagation §3 "declared"). A `documented`
 ...
 *  upstream write, so the read is EVENTUAL (doc: guarantee-propagation §2 "a cache hop with no declared
 *  invalidation contributes eventual"). Marked `est` — this is the honest default behaviour of an un-invalidated
