...
// (doc: load-stages §4-§9). These pin: a hold is constant, a ramp is monotone, ×1 = the level; cycles MULTIPLY
// within a generator (§5); the mean bills and the peak judges (§7); a flat generator is byte-for-byte today.

describe('cycleMultiplier — piecewise-linear k6 ramp semantics (doc: load-stages §4)', () => {
...
describe('generatorRate — λ(t) = level · Π cycles(t), the PRODUCT within a generator (doc: load-stages §5)', () => {
...
describe('derived mean & peak — the mean bills, the peak judges (doc: load-stages §7)', () => {
...
describe('shapeSeries — the ONE editor-preview sampler (doc: load-stages §11)', () => {
...
describe('the SACRED flat byte-identity (doc: load-stages §9)', () => {
...
describe('multi-origin SUM — a batch report is its own generator node (doc: load-stages §5.2)', () => {
...
describe('presets & the absorbed STRESS_DEFAULTS (doc: load-stages §16.3)', () => {
  ...
  // THE ONE-SHOT ENCODING (doc: load-stages §16.3 A/B) — a spike/ramp-up is a SHORT cycle whose period is its own
