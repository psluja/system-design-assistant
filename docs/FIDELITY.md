...
The V&V coverage matrix as a published report (doc: `docs/design/verification-and-validation.html`). It crosses every engine capability and modeling family with its evidence and prints the **honest** state: green only where a measured calibration residual OR an analytic anchor exists, and every remaining cell UNVALIDATED in plain sight. A visible gap is not a failure; a hidden one is. Regenerate with `pnpm fidelity`.
