...
Seam: `content/sda/src/analysis/uncertainty.ts`
...
- **Algorithms:** `content/sda/src/analysis/uncertainty.ts`, `engine/solver-contract/src/gpu/compile.ts`, `engine/solver-contract/src/gpu/fp32.ts`, `engine/solver-contract/src/gpu/webgpu.ts`
- **Docs:** none
- **E2E:** none (unit + differential: content/sda/src/analysis/uncertainty.test.ts and engine/solver-contract/src/gpu/differential.test.ts)
...
- **Algorithms:** `engine/solver-contract/src/native/search.ts`, `engine/solve/src/minizinc/search.ts`, `content/sda/src/analysis/robust.ts` (worlds mode)
- **Docs:** none
- **E2E:** `content/sda/src/analysis/optimize.e2e.test.ts`, `content/sda/src/analysis/sizing.e2e.test.ts`, `app/mcp/src/cqrs-escalation.e2e.test.ts`
- **Status:** shipped

### Capacity envelope

Seam: `content/sda/src/analysis/envelope.ts`
...
- **Algorithms:** `content/sda/src/analysis/envelope.ts`, `engine/solver-contract/src/native/search.ts`, `content/sda/src/analysis/queueing.ts`
- **Docs:** none
- **E2E:** `content/sda/src/analysis/envelope-des.e2e.test.ts`
- **Status:** shipped

### Delivery guarantees (consistency / ordering / delivery propagation)

Seam: `content/sda/src/vocabulary/guarantees.ts`
...
Seam: `content/sda/src/doc/design-doc.ts`
...
- **Algorithms:** `content/sda/src/analysis/system.ts`, `content/sda/src/analysis/queueing.ts`, `content/sda/src/analysis/sweep.ts`, `engine/sim/src/des.ts` (measured verdicts via doc-sim)
- **Docs:** none
...
Seam: `content/sda/src/vocabulary/manifest.ts`
...
- **Algorithms:** `engine/solve/src/network/build.ts` (scalar folding), `content/sda/src/analysis/sim.ts` (DES route means), `engine/sim/src/profile.ts` (load-curve arithmetic)
- **Docs:** none
- **E2E:** `content/sda/src/vocabulary/transform.e2e.test.ts`, `content/sda/src/vocabulary/generator.e2e.test.ts`
- **Status:** shipped
...
Seam: `content/sda/src/analysis/lag-slo.ts`
...
- **Algorithms:** `content/sda/src/analysis/queueing.ts` (the Dijkstra scalar lower bound), `engine/sim/src/des.ts` (measured async-inclusive lag)
- **Docs:** none
- **E2E:** `content/sda/src/analysis/lag-slo.e2e.test.ts`
- **Status:** shipped

### Load stages (traffic that changes over time + peak-aware verdicts)

Seam: `content/sda/src/analysis/load-stages.ts`
...
- **Algorithms:** `content/sda/src/analysis/load-stages.ts` (the multi-cycle rate product), `content/sda/src/analysis/time-sweep.ts` (the Tier-1 quasi-static sweep + per-node peak), `content/sda/src/analysis/two-tier.ts` (propose/prove), `content/sda/src/analysis/sim.ts` (DES lowering)
- **Docs:** none
...
Seam: `content/sda/src/analysis/reliability.ts`
...
- **Surfaces:** mcp (reliability, app/mcp/src/reliability.ts), design doc (reliability section via content/sda/src/doc/design-doc.ts)
- **Algorithms:** `content/sda/src/analysis/system.ts` (the series-product availability it interprets)
- **Docs:** none (primary AWS sources are cited inline below)
- **E2E:** none (unit: content/sda/src/analysis/reliability.test.ts, `app/mcp/src/reliability.test.ts)`
- **Status:** shipped

### Request classes (multi-commodity flows over one topology)

Seam: `content/sda/src/analysis/request-class.ts`
...
- **Docs:** none
- **E2E:** `content/sda/src/analysis/fanin.e2e.test.ts` (the mesh case)
...
Seam: `content/sda/src/analysis/verdict.ts`
...
- **Algorithms:** `content/sda/src/analysis/queueing.ts`, `engine/sim/src/des.ts`, `engine/sim/src/stats.ts`
- **Docs:** none
- **E2E:** `content/sda/src/analysis/tail.e2e.test.ts`, `content/sda/src/analysis/response-latency.e2e.test.ts`, `app/web/src/latency-chips.e2e.test.ts`
- **Status:** shipped

### Retry feedback & goodput collapse

Seam: `content/sda/src/analysis/sim.ts`
...
- **Algorithms:** `engine/sim/src/des.ts`, `content/sda/src/analysis/sim.ts`
- **Docs:** none
...
Seam: `content/sda/src/analysis/robust.ts`
...
- **Algorithms:** `content/sda/src/analysis/robust.ts`, `engine/solver-contract/src/native/search.ts`
- **Docs:** none
- **E2E:** none (property + unit: content/sda/src/analysis/robustness.property.test.ts)
- **Status:** shipped (surfaced through the search tools, no standalone tool name)

### Synthesis & compare-options (generate, size, rank)

Seam: `content/sda/src/analysis/synthesize.ts`
...
- **Docs:** none
- **E2E:** `content/sda/src/analysis/synthesize.e2e.test.ts`
- **Status:** shipped

### System evaluation & roll-up

Seam: `content/sda/src/analysis/system.ts`
...
- **Algorithms:** `content/sda/src/analysis/system.ts`, `engine/solve/src/fixpoint/solve.ts`, `engine/solve/src/network/build.ts`, `content/sda/src/analysis/queueing.ts`
- **Docs:** none
- **E2E:** `content/sda/src/analysis/architectures.e2e.test.ts`, `content/sda/src/analysis/system.e2e.test.ts`, `app/web/src/value-loop.e2e.test.ts`
- **Status:** shipped

### System promise (whole-design cost ceiling)

Seam: `content/sda/src/analysis/system-promise.ts`
...
- **Algorithms:** `content/sda/src/analysis/system.ts`, `engine/solver-contract/src/native/search.ts`, `engine/solve/src/minizinc/search.ts` (Objective.total — one sum shared by verdict and search)
...
Seam: `content/sda/src/analysis/scenario.ts`
...
- **Algorithms:** `engine/solve/src/fixpoint/solve.ts`, `content/sda/src/analysis/envelope.ts` (the trio derives demand from the envelope)
- **Docs:** none
