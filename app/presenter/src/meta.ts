...
export const KEY_INFO: Record<string, KeyInfo> = {
  ...
  // RETRY POLICY (doc: retry-feedback) — caller-side knobs on the CALLING node. Default 0 ⇒ no retries (today's
  ...
  // DES-FED OUTCOME SLOs (doc: retry-feedback §3) — answered only by the simulation (like the p99 tail).
