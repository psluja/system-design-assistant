...
// them — and, per the anti-drift rule (doc: flow-transformations-r2 §2), the rate is computed with the ENGINE's own
...
/** Where an OUT-side transform was RESOLVED FROM (doc: flow-transformations-r2 §5) — so a UI can mark provenance
...
/** Resolve the OUT-side transform of ONE wire with its full precedence (doc: flow-transformations-r2 §5):
...
function toneOf(t: Transform): TransformTone {
  switch (t.kind) {
    ...
    case 'generate':
      return 'gen'; // an ORIGIN, not a reshaping (doc: load-curves §3) — its own tone
  }
}
