 ...
 *
 * RETRY OUTCOME (doc: retry-feedback §3). Past saturation real systems don't plateau — clients time out, retry,
...
/** The System rows for per-node RESPONSE latency (doc: latency-semantics-v2 §4) — one row per REQUIREMENT-BEARING
...
export interface SummaryInput {
  ...
  /** Declared flow-scoped LAG verdicts (doc: latency-semantics-v2 §3), already computed by the shell via
  ...
  /** PEAK-AWARE (doc: load-stages R4): each node's WORST-WINDOW ρ + instant from the Tier-1 sweep (content
...
export function summarySections(input: SummaryInput): SummarySection[] {
  ...
  // Response latency · per node (doc: latency-semantics-v2 §4) — the request→response tail a caller of each
  ...
  // Propagation lag · flow-scoped (doc: latency-semantics-v2 §3) — one row per declared CDC/replication deadline
