...
// THE IDEAL LAYOUT (doc: ideal-layout, TASK-88) — semantic placement + a seeded search scored on real routes. The
...
// THE RESTING HANDSHAKE (doc: ideal-layout §3.6) — the shell-agnostic '✨ Ideal layout' driver both shells share:
...
// THE IDEAL LAYOUT — the GPU PROPOSER (doc: ideal-layout §3.3–3.4, TASK-88 R3). The fp32 straight-line proxy
...
// THE AMBIENT TWO-TIER READ-OUT (doc: load-stages §10) — the ρ-envelope strip + worst-window callout + cost
...
// The assumption model (doc: assumption-model §3, §7) — the envelope-by-default headline and the worlds matrix +
...
// Range TEXT I/O — display + parse + validate for uncertainty ranges (doc: uncertainty-monte-carlo §2), shared so
...
// the qualitative-guarantee (consistency/ordering/delivery) surface every shell renders (doc: guarantee-propagation).
export {
  flowGuaranteeLines,
