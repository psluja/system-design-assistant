...
// zero rows in the panels (doc: guarantee-propagation §4 "no requirement ⇒ the whole feature is silent").
...
export function flowGuaranteeLines(input: GuaranteeViewInput): FlowGuaranteeLine[] {
  ...
  const key = (s: string, t: string, d: string): string => `${s} ${t} ${d}`;
