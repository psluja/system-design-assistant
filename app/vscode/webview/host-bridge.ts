 ...
 * RETRY OUTCOME: goodput/failures/amplification + whether a policy is declared, carried
...
/** The ambient Monte-Carlo view the background loop produces: the latest result (null while the first
 ...
 * • `pick` — the QUICK-ADD picker: hand the host the legality-filtered options (computed by App via
...
export interface HostBridge {
  ...
  /** Subscribe to ambient Monte-Carlo updates; returns an unsubscribe. Latest-wins, off-thread. */
  onUncertainty(handler: (unc: UncertaintyView | null) => void): () => void;
  /** Subscribe to the ambient two-tier transient; returns an unsubscribe. Latest-wins,
