...
// The post-insert overlap predicate behind the "Tidy?" offer (TASK-71) — the SAME pure geometry the web shell uses.
...
export default function App({ studio, bridge }: { studio: Studio; bridge: HostBridge }): JSX.Element {
  ...
  // AMBIENT UNCERTAINTY (TASK-81) — the background Monte-Carlo view (fp32 preview / fp64 confirmed), fed by
  ...
  // AMBIENT TWO-TIER TRANSIENT (doc: load-stages §10) — the ρ-envelope + worst window (Tier-1) and the survival
  ...
  // ASSUMPTION MODEL (doc: assumption-model §3, §7) — the capacity ENVELOPE (default answer) and the all-world
  ...
  // The GUARANTEE view input (doc: guarantee-propagation §4) — the built graph + solved value + declared
  ...
  // Per-flow LAG verdicts (doc: latency-semantics-v2 §3), SCALAR pass — the async-inclusive propagation deadlines
  ...
  const wireProblems = useMemo<readonly WireProblem[]>(() => {
    ...
    // GUARANTEE violations flow into the SAME Problems panel (doc: guarantee-propagation §4). A broken qualitative
  ...
  // A transient "Tidy?" offer after a picker insert whose new node landed ON another (TASK-71). It NEVER
  ...
  // THE IDEAL LAYOUT (doc: ideal-layout §3.6, the resting handshake) — the web shell's ✨ pipeline run HERE, in the
  ...
  useEffect(() => {
    ...
    // Which (node, port) pairs are WIRED — so the renderer shows the inline "+" only on OPEN ports (TASK-71).
  ...
  // The QUICK-ADD picker (TASK-63 "canvas smoothness"): open the native QuickPick with the shared, legality-filtered
  ...
  // The inline "+" on an OPEN port (TASK-71): open the SAME native pick as drop-to-pick for that port, placing the
  ...
  // '✨ Ideal layout' (doc: ideal-layout): Tidy instantly (the floor), then polish in the background and morph to
  ...
  const buildDesignDoc = (): { markdown: string; title: string } | null => {
    ...
    const md = generateDesignDoc({
      ...
      // The retry outcome (doc: retry-feedback) so §4 reports goodput vs offered when a policy is live.
      retry: sim && sim.goodputRps !== undefined && sim.errorRate !== undefined && sim.amplification !== undefined && (sim.retryPolicy || sim.amplification > 1 || sim.errorRate > 0)
        ...
        : undefined,
      // Per-node SIMULATED response percentiles (doc: latency-semantics-v2 §4; TASK-87 F3) from the ambient DES —
      ...
      // Per-flow LAG verdicts (doc: latency-semantics-v2 §3) — the async-inclusive deadlines, judged by the scalar
  ...
  const summarySections = useMemo<readonly SummarySection[]>(
    () => {
      ...
      // Append the per-flow GUARANTEE sections (doc: guarantee-propagation §4) — one section per flow with a
      ...
      const gSections: SummarySection[] = guaranteeInput
        ...
        : [];
      // AMBIENT UNCERTAINTY (TASK-81) — the Monte-Carlo distribution block, from the SHARED presenter (identical to
      ...
      // AMBIENT TWO-TIER TRANSIENT (doc: load-stages §10) — the ρ-envelope + worst window + survival verdict, from the
      ...
      // ASSUMPTION MODEL (doc: assumption-model §3, §7) — the envelope-by-default headline and the worlds matrix,
  ...
  const nodeDetail = useMemo<NodeDetail>(() => {
    ...
    // The selected node's simulated RESPONSE tail from the last run (doc: latency-semantics-v2 §4) — the presenter
  ...
  return (
    ...
    <main className={'cv canvas-only' + (morphing ? ' morphing' : '')} ref={cvRef} onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; }} onDrop={onDrop}>
      ...
      {/* Empty-canvas ghost CTA (TASK-63, n8n's "Add first step"): a centred dashed affordance that opens the
