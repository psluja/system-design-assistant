...
// The QUICK-ADD picker round-trip. App computes the legality-filtered options and awaits `bridge.pick`;
...
function runSimulate(): void {
  ...
  try {
    ...
    // FLOW-SCOPED LAG: hand the declared (source, terminal) pairs to the worker so the
    ...
    void spawnSimWorker()
      .then((w) => {
        ...
        w.onmessage = (e: MessageEvent<SimWorkerResponse>) => {
          ...
          w.terminate();
          const d = e.data;
          // The retry-outcome fields ride along the tail so the shared summary + verdicts
          ...
          const retry = d.ok
            ...
            : {};
          // Per-node response tails + declared lags ride the same tail so the
...
// ── THE AMBIENT TWO-TIER TRANSIENT ─────────────────────────────────────────────────────
...
function Root(): JSX.Element {
    ...
    const onMessage = (e: MessageEvent): void => {
      ...
      switch (msg.type) {
        ...
        case 'wireSuggestion':
          ...
          break;
        // The native quick-add QuickPick answered: resolve the pending pick by token so App can place +
  ...
  // Debounce the AMBIENT UNCERTAINTY loop after each change, the same latest-wins/off-thread discipline
  ...
  // Debounce the AMBIENT TWO-TIER TRANSIENT after each change — the resting refine, a touch
...
function sampleProject(): ProjectDoc {
  ...
  // A per-port TRANSFORM so the dev harness shows the R2 edge pill: the API emits
