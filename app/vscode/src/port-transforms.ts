...
// Pure, vscode-free catalog + helpers for NATIVE per-port TRANSFORM editing (doc: flow-transformations-r2 §4).
...
export function validateTransformValue(kind: Transform['kind'], value: number): string | null {
  ...
  // (doc: load-curves §3). The QuickPick does not yet author generators (a load-curves R4 surface); this keeps
...
/** One declared GUARANTEE contribution on a port (doc: guarantee-propagation §4 "Inspector · a read-only Guarantees
 *  block on every port"): the dimension, the token this port contributes, and its provenance badge — read-only, from
...
export function portRowsFor(text: string, node: string): readonly PortRow[] {
  ...
  return man.ports.map((p) => {
    ...
    // The port's DECLARED guarantee contributions with provenance (doc: guarantee-propagation) — recovered from the
...
/** One wire row for the `sda.setWireTransform` QuickPick (doc: flow-transformations-r2 §5): the wire's endpoints and
...
/**
 * Parse the compact generator syntax into a level + {@link Cycle}s (doc: load-stages §11). Guided-error throughout —
...
/** A preset's cycles as the compact syntax (doc: load-stages §11, §16.3) — the seed a preset pick pre-fills the
