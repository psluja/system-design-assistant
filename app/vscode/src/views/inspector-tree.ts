...
// BY THE REGISTRY ROLE AXIS (doc: assumption-model §2, shared presenter `knobGroupOf`) rather than a flat
  ...
  getTreeItem(element: Item): vscode.TreeItem {
    switch (element.kind) {
      ...
      case 'knob': {
        ...
        // (doc: uncertainty-monte-carlo §2) is appended as `· ±(lo–hi)` so a ranged knob reads distinctly, and the
...
function knobTooltip(node: string, knob: KnobRow, range: Range | undefined, world: ScenarioOverride | undefined): vscode.MarkdownString {
  ...
  // The declared uncertainty RANGE (doc: uncertainty-monte-carlo §2) — the honest admission this soft input is not a
...
function portTooltip(node: string, row: PortRow): vscode.MarkdownString {
  ...
  // The DECLARED guarantee contributions on this port, with provenance (doc: guarantee-propagation §4). Read-only,
