...
/**
 * Set (upsert) OR remove one per-FLOW guarantee requirement, and return the
...
/**
 * Set (upsert) OR clear one PORT TRANSFORM override on one node, and return the
...
/**
 * Set (upsert) OR clear one per-instance uncertainty RANGE on a config key, from
...
/**
 * Set (upsert) OR clear one per-WIRE transform override, and return the
