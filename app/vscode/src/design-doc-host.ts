 ...
 *
 * DES-IN-THE-DOC (TASK-87 F3): the host runs the discrete-event simulation SYNCHRONOUSLY (it is deterministic and
...
export function buildDesignDocText(text: string, format: 'html' | 'markdown'): GeneratedDoc | null {
  ...
  // DES-IN-THE-DOC (TASK-87 F3): run the simulation synchronously so the file carries the measured tail / per-node
