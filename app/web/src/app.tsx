...
export default function App() {
  ...
  // THE ACTIVE-WORLD LENS (doc: assumption-model §7.1) — Studio UI state (out of the doc), subscribed on the SAME
  ...
  // THE AMBIENT TWO-TIER TRANSIENT (doc: load-stages §10) — no button: whenever a generator declares periodic
  ...
  // PEAK-AWARE PER-NODE LOAD (doc: load-stages R4 — "the usual place") — each node's WORST-WINDOW ρ + instant from
  ...
  // AMBIENT UNCERTAINTY (TASK-81) — Monte Carlo recomputed OFF-thread on every design change, shown in the System
  ...
  // ASSUMPTION MODEL (doc: assumption-model §3, §7) — the capacity ENVELOPE (the default answer, no demand needed)
  ...
  // The bill DEPTH (doc-10): compute + the most-missed egress line + the committed-pricing scenarios — from the
  ...
  // A transient "Tidy?" offer shown after a picker insert whose new node landed ON another (TASK-71). It NEVER
  ...
  // THE IDEAL LAYOUT (doc: ideal-layout §3.6, the resting handshake) — the pipeline the single 'Tidy' button runs.
  ...
  // The in-canvas quick-add picker (TASK-63): opened by a wire dropped on the pane (with a port context),
  ...
  // The TRANSFORM editor popover (R2, doc: flow-transformations-r2 §4; wire level TASK-75 §5). Opened by a click on
  ...
  // The uncertainty ± RANGE editor popover (doc: uncertainty-monte-carlo §4) — opened from a Configuration knob's ±
  ...
  // The right Inspector is collapsible (TASK-73): a slim grip on its left edge, mirroring the System drawer.
  ...
  // First-run onboarding (TASK-73): the "Start here" card shows once, only on a fresh profile (`sda.onboarded`
  ...
  // SCENARIO-FIRST AUTHORING (doc: assumption-model §7.1) — a knob edit writes INTO the active world when that world
  ...
  // ── GUARANTEE PROPAGATION (doc: guarantee-propagation §4) — the qualitative-guarantee surface, all read from the
  ...
  // Per-flow LAG verdicts (doc: latency-semantics-v2 §3) — the async-inclusive propagation deadlines. When a sim has
  ...
  // Problems panel as the numeric ones (doc: guarantee-propagation §4). The `fix` is the computed remediation (R2 —
  ...
  useEffect(() => {
    ...
    // Which (node, port) pairs are WIRED — so the node renderer shows the inline "+" only on OPEN ports (TASK-71).
    ...
    // SHAPED ORIGINS (doc: load-stages §11, honest visibility) — the nodes whose generator declares non-flat cycles,
    ...
    const compNodes: Node[] = doc.instances.map((inst) => {
      ...
      // A SHAPED GENERATOR (doc: load-stages §11) — a distinct ⚡ chip carrying the shape silhouette, so a
    ...
    const flowEdges = doc.wires.map((w, i) => ({
      ...
      data: {
        ...
        // The guarantee STRIP segment for this wire (doc: guarantee-propagation §4) — present only when a declared
  ...
  // legality-filtered quick-add picker for exactly that port (drop-to-pick, TASK-63).
  ...
  // After a picker-driven insert, offer "Tidy?" if the NEW node's box intersects any other (TASK-71). Reads the
  ...
  // The inline "+" on an OPEN port (TASK-71): open the SAME legality-filtered picker as drop-to-pick, anchored on
  ...
  // A click on an edge transform PILL (R2; wire level TASK-75 §5): open the transform editor. An OUT-side pill edits
  ...
  // Open the uncertainty ± RANGE editor from a Configuration knob's ± affordance (doc: uncertainty-monte-carlo §4).
  ...
  const buildDesignDocInput = (): DesignDocInput | null => {
    ...
    return {
      ...
      // The retry outcome (doc: retry-feedback) so §4 reports goodput vs offered + error rate when a policy is live.
      retry: sim && (sim.retryPolicy || sim.amplification > 1 || sim.errorRate > 0)
        ...
        : undefined,
      // Per-flow qualitative guarantee verdicts (doc: guarantee-propagation §4) — the SAME `guaranteeVerdicts` the
      ...
      guaranteeVerdicts: graph && doc.guaranteeSlos.length > 0
        ...
        : undefined,
      // Per-flow LAG verdicts (doc: latency-semantics-v2 §3), RESOLVED by the sim when one has run — the SAME `lagV`
      ...
      // Per-node SIMULATED response percentiles (doc: latency-semantics-v2 §4) — the DES tail for §4's requirement-
  ...
  // '✨ Ideal layout' (doc: ideal-layout): Tidy instantly (the floor), then polish in the background and morph to the
  ...
  const idealLayout = (): void => {
    ...
    // THE GPU PROPOSER (doc: ideal-layout §3.3, TASK-88 R3). When a real WebGPU device is present, inject the fp32
  ...
  // that meets every SLO, then apply it as commands. As of TASK-79 phase 3 the DEFAULT is the native in-process
  ...
  // The ONE backward-solve action (doc-11 §3d). repair = make it meet every SLO (the old "Auto-fix"); optimize =
  ...
  const runSimulate = (): void => {
    ...
    try {
      ...
      setBusy('sim');
      // FLOW-SCOPED LAG (doc: latency-semantics-v2 §3): hand the declared (source, terminal) pairs to the worker so
  ...
  // THE AMBIENT TWO-TIER runner (doc: load-stages §10) — same worker discipline as runSimulate (off-thread,
  ...
  // THE AMBIENT TWO-TIER TRANSIENT (doc: load-stages §10) — the resting refine: debounced a touch after the sim so
  ...
  // ── AMBIENT UNCERTAINTY LOOP (TASK-81) ───────────────────────────────────────────────────────────────────────
  ...
  // ── AMBIENT ENVELOPE + WORLDS LOOP (doc: assumption-model §3, §7.2) ───────────────────────────────────────────
  ...
  // RESET ONE WORLD (doc: assumption-model §5.3 — "reset means reset") — the NON-preserving twin of deriveTrio. A
    ...
    <div className="app">
      ...
      <div className="mid">
        ...
        <main className={'cv' + (morphing ? ' morphing' : '')} ref={cvRef} onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; }} onDrop={onDrop}>
          ...
          {/* First-run onboarding (TASK-73): three honest quick-actions + the bundled CQRS example. Shown once on
        ...
        {/* Inspector collapse grip (TASK-73) — the middle column of the .work grid; mirrors the System drawer's
