 ...
 * edit callback so a pill click opens the RIGHT port's transform editor. */
...
export type FlowEdgeData = {
  ...
  /** The GUARANTEE STRIP segment for this edge — present ONLY when the SELECTED
...
function SdaNode({ data }: NodeProps): JSX.Element {
  ...
  // The inline "+" on an OPEN (unwired) port: a hover-revealed affordance that opens the SAME
...
function FlowEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, data }: EdgeProps): JSX.Element {
  ...
  // R2 — RATES ON THE WIRE. A pill click routes to the shell's transform editor
  ...
  return (
    <>
      ...
      {/* GUARANTEE STRIP — a slim band riding UNDER the wire, drawn ONLY when the
