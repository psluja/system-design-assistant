...
// The SYSTEM lens, extracted from app.tsx — the whole-design blocks of the
...
export function SystemPanel({
  ...
  /** The ambient two-tier transient — null when no generator declares cycles (no-filler). */
  twoTier: TwoTierResult | null;
  /** PEAK-AWARE per-node load: each node's worst-window ρ + instant from the Tier-1 sweep, so
  ...
  const flowSections = flows.map((fl, i) => {
    ...
    return (
      <div className="sec" key={`flow${i}`}>
        ...
        {/* GUARANTEE line + requirements — hidden for the first release (owner, 2026-07-11). */}
        {(() => {
