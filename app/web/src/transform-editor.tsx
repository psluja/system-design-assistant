...
const FUNCTIONS: ReadonlyArray<{ readonly fn: Fn; readonly label: string; readonly hint: string; readonly help: string }> = [
  ...
  // The sixth family member: the port ORIGINATES traffic instead of reshaping it. PORT
...
/** The preset dropdown entries — each pre-fills the table from `LOAD_STAGES_PRESETS`,
 ...
 * architect always knows WHICH level a change touches. */
...
// ── The editable stages table state. Cells are held as STRINGS so a partly-typed value never
...
/** Validate the whole generator — the shared `cyclesProblem` over the parsed table, plus the
...
export function TransformEditor({ target, current, x, y, onApply, onClose }: TransformEditorProps): JSX.Element {
  ...
  // port default. A wire is titled by its endpoints; a port by node.port.
  ...
  // The read-only preview — recomputed on every keystroke from the SAME sampler the engine
  ...
  return (
    <>
      ...
      <div className={'ai-pop tf-pop' + (isGen ? ' tf-pop-gen' : '')} style={{ left: x, top: y, right: 'auto' }} onMouseDown={(e) => e.stopPropagation()}>
        ...
        {/* THE GENERATOR — level + preset + the k6 stages table + preview + disable. */}
        {isGen && (
...
/** The read-only shape sparkline — an SVG polyline of the `×1`-anchored multiplier
