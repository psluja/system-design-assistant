...
// THE ASSUMPTION MODEL over MCP (doc: assumption-model §8) — the minimal surface so the owner can DRIVE the whole
...
export function buildAssumptionTools(studio: Studio, registry: Registry, solvers: SolverBindings): AsyncToolDef[] {
  ...
  return [
    ...
    },
    {
      ...
      description:
        "Set one FACT-ASSUMPTION override in a named world (doc: assumption-model §7): {scenario, node, key, value}. Only a fact-assumption key is overridable — a resource limit is refused (change it with set_config / the Improve search), a computed result is an output (not settable), an SLO is a promise (set_slo). The error names the key's actual role and the right surface. Replaces the value if the (node,key) is already overridden in that world. e.g. {scenario:\"peak\", node:\"client\", key:\"throughput\", value:12000}",
