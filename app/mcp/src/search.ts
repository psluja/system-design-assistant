...
// The backward-search MCP tools. These are async — they await
...
//
// The tools depend on the solver CONTRACT (`SolverBindings`), not on a
// concrete engine: the composition root (composition.ts) binds an adapter and hands it in. As of 
...
// differently — conflating them is a lie. And a did-not-converge is NOT always a time limit: the native
// solver mostly declines for a STRUCTURAL reason (an SLO outside its monotone class), which it names via the
// SearchResult's `reason`. We surface that true cause verbatim — never a cause dressed as a timeout.
//
...
export function buildSearchTools(studio: Studio, solvers: SolverBindings, reference?: ReferenceSolver, audience: SolverAudience = 'agent'): AsyncToolDef[] {
   ...
   * mislabels a structural decline as a timeout. */
  ...
  // a throw and a time limit are different facts. It surfaces the error honestly as an internal error.
