...
export const SEARCH_MESSAGES = {
  ...
  /** The multi-class honest decline (doc: request-classes §5.2) — search under declared request classes. */
  multiClassDecline: {
    agent: 'multi-class search is not yet available — request classes break per-class monotonicity at shared saturated nodes, so the native solver must first be certified by the oracle harness’s class axis (doc: request-classes §5.2). Evaluate per class instead, remove the request classes to search the single-river design, or set the provisioning knobs manually.',
    human: 'searching a design with request classes is not available yet — classes break the solver’s per-class monotonicity at shared saturated nodes (doc: request-classes §5.2). Remove the request classes to search the single-flow design, or set the provisioning knobs manually.',
  },
