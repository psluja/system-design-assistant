...
// The TIME engine as an MCP tool (doc-4 §3b). The scalar forward pass cannot judge a tail, so a percentile
  ...
  return [
    {
      ...
      annotations: READS,
      run: () => {
        // MULTI-CLASS DES — the honest decline (doc: request-classes §6). The simulator routes over the class-BLIND
        ...
        if (studio.project().requestClasses.length > 0)
          return fail(
            'per-class simulation is not yet available — request classes need per-class routing and (node,class) response reservoirs in the DES (doc: request-classes §6). Use evaluate for the per-class scalar picture, or remove the request classes to simulate the single-river design.',
          );
        const g = studio.graph();
        if (!g.ok) return fail('design has build errors — resolve those first');

        // FLOW-SCOPED LAG (doc: latency-semantics-v2 §3): declare the pairs to measure so the ONE run also samples
        ...
        // Per-flow LAG verdicts, now RESOLVED by the run (doc: latency-semantics-v2 §3): the DES measured the true
        ...
        // LOAD STAGES — the ambient two-tier transient (doc: load-stages §10): when a generator declares periodic
        ...
        return json(
            {
              ...
              // Retry-feedback outcome accounting (doc: retry-feedback §3). Without a retry policy these degenerate to
...
/**
 * Shape the two-tier transient (doc: load-stages §10) into the guided JSON `simulate` returns under `loadStages`.
