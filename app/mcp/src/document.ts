...
// actual output: a Markdown design document
...
// recomputation on either surface.
  ...
  return [
    {
      ...
      run: (a) => {
        ...
        // DES-IN-THE-DOC: run the discrete-event simulation ONCE and EMBED the measured tail / per-node
        ...
        const input: DesignDocInput = {
          ...
          // Per-flow qualitative guarantee verdicts — the SAME computation `evaluate`
          ...
          guaranteeVerdicts: g.ok && proj.guaranteeSlos.length > 0
            ...
            : undefined,
          // Per-flow LAG verdicts — the async-inclusive propagation deadlines. When the
