...
/** The closed set of RESHAPING flow-transform functions. The sixth family member,
 * `generate`, ORIGINATES flow instead of reshaping it and takes a level (+ optional cycles),
...
/** The six shipped generator PRESETS an agent can name on `set_transform fn:generate`
...
/** The MCP toolset over a Studio — the command surface made callable by any agent. */
export function buildTools(studio: Studio): ToolDef[] {
  ...
  const evaluateResult = (): ToolResult => {
    ...
    // a CYCLIC single-river flow has no finite steady state — the throughput feeds back into itself, so
    ...
    // UNDER REQUEST CLASSES, the queueing-aware correction, the system roll-up, the tail
    ...
    // Per-flow qualitative GUARANTEES — the SAME propagation the canvas/design-doc
    ...
    const guaranteeReqs = g.ok && proj.guaranteeSlos.length > 0
      ...
      : [];
    // JUDGED lag requirements — the SCALAR pass (no sim on the `evaluate` path): each
    ...
    return json(
      {
          ...
          systemPromiseVerdicts: systemReqs.length > 0
            ...
            : undefined,
          // Per-flow LAG verdicts — the async-inclusive propagation deadline judged by
  ...
  return [
    ...
    },
    {
      ...
      run: (a) => {
        ...
        // A port's DECLARED guarantee contribution with its provenance — so the AI sees
    ...
    },
    {
      ...
      run: (a) => {
        ...
        // F6 (audit swallow bug): a named port that does NOT exist made a phantom wire (illegalWire reads
    ...
    },
    {
      ...
      run: (a) => {
        ...
        if (fn === 'generate') {
          // THE GENERATOR: the port ORIGINATES `value` req/s (the BASELINE). Guided rules:
          ...
          if (a.cycles !== undefined && a.cycles !== null) {
            ...
            // A PRESET names a shipped shape — the one-call on-ramp. `flat` clears the shape
    ...
    // ── REQUEST CLASSES — declare the named multi-commodity flows over a shared,
