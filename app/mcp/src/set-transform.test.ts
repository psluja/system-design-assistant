...
// set_transform — the MCP surface for per-port flow transforms. Guided errors (the
...
// set_wire_transform — the MCP surface for per-WIRE routing splits. One out port
...
// set_transform fn:'generate' — the sixth family member: the port ORIGINATES traffic.
...
describe('MCP set_transform fn:generate — the generator port function', () => {
  ...
  // PRESETS — a preset NAME pre-fills the cycles in one call, the AI's on-ramp to the same
