...
/**
 * The command core (doc-9 §1): the single source of truth. Holds the document, applies commands (with
...
export class Studio {
  ...
  /** THE ACTIVE-WORLD LENS (doc: assumption-model §7.1) — which named world scopes the view. Deliberately OUT of
  ...
  /** The declared request classes lowered to the engine's `RequestClass[]` (doc: request-classes §3), or undefined
  ...
  /** The per-instance class context content needs at instantiate time (doc: request-classes §4.1): present ONLY
        ...
        error: g.error.map((e) =>
          e.kind === 'unknown-type'
            ...
            : e.kind === 'generate-on-in-port'
              ? `port "${String(e.port)}": a generator (generate) originates traffic, so it lives on an OUT port — move it to one of the node's out ports (doc: load-curves §3)`
              : e.kind === 'generate-on-edge'
                ? `wire: a generator (generate) is a PORT function — set it on the source out port (set_transform), not on a wire (doc: load-curves §3)`
                : JSON.stringify(e),
        ),
      };
    }
