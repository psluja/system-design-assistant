 ...
 *  level (+ whether cycles ride it) — `generate` carries no scalar `value` (doc: load-stages §4). */
...
/**
 * The command surface (doc-9 §2) — the one way to change a design. Every client (MCP, UI, WebMCP)
...
export type Command =
  ...
  // REQUEST CLASSES (doc: request-classes §7.1) — declare/edit the named multi-commodity flows over the topology.
  ...
  // NAMED WORLDS (doc: assumption-model §4, §7) — declare/edit the scenarios (points in the assumption space).
...
export function apply(doc: ProjectDoc, cmd: Command, knownTypes: ReadonlySet<string>): Result<Applied, string> {
  ...
  switch (cmd.kind) {
    ...
    case 'renameNode': {
      ...
      // Lag requirements are keyed by (source, terminal) too (doc: latency-semantics-v2 §3) — rewrite them with the
      ...
      // dangling its membership into a build error (doc: request-classes §3).
    ...
    case 'setRange': {
      // A per-instance uncertainty RANGE on a config key (doc: uncertainty-monte-carlo §2) — the additive, undoable
    ...
    case 'setTransform': {
      // A per-instance PORT TRANSFORM override (doc: flow-transformations), by port NAME — the additive, undoable
    ...
    case 'setWireTransform': {
      // A per-WIRE OUT-side transform override (doc: flow-transformations-r2 §5) — the routing-split twin of
      ...
      // A GENERATOR is a PORT function in R1 (doc: load-curves §3) — refused here with the fix named, before the
      // build would refuse it structurally (generate-on-edge).
      if (cmd.transform !== null && cmd.transform.kind === 'generate') return err('a generator is a PORT function — set it on the source OUT port (setTransform), not on a wire (doc: load-curves §3)');
      let found = false;
      const wires = doc.wires.map((w) => {
        if (w.from[0] === cmd.from[0] && w.from[1] === cmd.from[1] && w.to[0] === cmd.to[0] && w.to[1] === cmd.to[1]) {
          found = true;
    ...
    case 'setGuaranteeSlo': {
      // A per-FLOW guarantee requirement (doc: guarantee-propagation §4). Keyed by (source, terminal, dimension):
    ...
    case 'setLagSlo': {
      // A per-FLOW propagation-lag requirement (doc: latency-semantics-v2 §3). Keyed by (source, terminal): ONE lag
    ...
    case 'setClassMembership': {
      // Toggle ONE wire's membership in a class (doc: request-classes §7). A wire may belong to MANY classes
